...
cmake -B build -S . -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER="clang++" -DCMAKE_C_COMPILER="clang" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
...
clang++ -E fixups.hpp | [ `wc -l` = 8 ] || { FAILED=1; echo 'FAILED: nothing allowed in `fixups.hpp` apart from preprocessor directives'; }
